...
# Locate renderControlModal start
start_marker = "function renderControlModal(ch, name) {"
end_marker = "contentEl.innerHTML = html;"

if start_marker in content and end_marker in content:
    start_pos = content.find(start_marker)
    end_pos = content.find(end_marker) + len(end_marker)
    
    clean_func = """function renderControlModal(ch, name) {
        ...
        if (m.type === 'iot627') {
          ...
          <div style="flex:1; min-width:248px; max-width:268px; background:#ffffff; border:1.5px solid var(--border); border-radius:8px; color:#1e293b; font-family:var(--font-main); display:flex; flex-direction:column; justify-content:space-between; box-shadow:0 4px 10px rgba(0,0,0,0.06); overflow:hidden;">
             <div>
                <div style="background:var(--accent); text-align:center; padding:8px 34px 8px 10px; font-size:14px; font-weight:700; color:#ffffff; letter-spacing:1px; position:relative;">
                   電錶資訊
                   <span title="${uOnline ? '連線中' : '斷線'}" style="position:absolute; right:12px; top:50%; transform:translateY(-50%); width:9px; height:9px; border-radius:50%; background:${pOnline ? 'var(--ok)' : 'var(--alarm)'}; display:inline-block; box-shadow:0 0 0 2px rgba(255,255,255,0.18); animation:${pOnline ? 'pls 2s infinite' : 'blink 1.5s step-start infinite'};"></span>
                </div>
                <div style="display:flex; flex-direction:column; gap:8px; padding:12px 14px 8px 14px; font-size:13px; font-weight:700;">
                   <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #e2e8f0; padding-bottom:6px;">
    ...
    new_content = content[:start_pos] + clean_func + content[end_pos:]
    with open(r'local_web\templates\index.html', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("RESTORE SUCCESS: index.html 100% restored!")
else:
    print(f"FAILED TO LOCATE POSITIONS: start_pos={content.find(start_marker)}, end_pos={content.find(end_marker)}")

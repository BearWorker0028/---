...
TEMP_TABLE = "gw1_temp_status"     # IoT-627 溫控器
METER_TABLE = "gw1_meter_status"   # SPM-3 電表

# 建立背景寫入 Queue (避免網路延遲阻塞 Modbus 輪詢)
db_queue = queue.Queue()

# ── 1. 系統與通訊設定 ──
TCP_SERVER_PORT = 8801              # GCP 開放的 TCP Server Port
...
# ── 5. 遠端控制命令佇列 ──
# 前端（app.py）把命令寫進 Supabase 的 device_commands 表；
# 這裡透過已經建立好的 W610 連線執行寫入，執行完立刻回報結果。
    ...
    for cmd in commands:
        ...
        if not dev:
            ...
            try:
                ...
                if success:
                    logging.info(f"✅ [{ch}] 控制溫度設定已寫入: {cmd['value']}°C")
                else:
                    error_message = '寫入失敗或逾時'
            except ConnectionDeadError:
                # 連線已確定失效，往外拋出讓外層迴圈觸發重連，不要當成單純的命令失敗吞掉
                raise
            except Exception as e:
            ...
            try:
                ...
                if success:
                    logging.info(f"✅ [{ch}] 停控模式(A801)已寫入: {'開' if raw_value == 1 else '停'}")
                else:
                    error_message = '寫入失敗或逾時'
            except ConnectionDeadError:
                raise
